...
func writeOEMBrewService(target string, vendor string, pkgs []string) error {
	...
	scriptDir := filepath.Join(etcDir, "dakota", "oem")
...
MARKER="$HOME/.config/dakota/oem-setup-done"
if [[ -f "$MARKER" ]]; then
    exit 0
fi
...
# Ensure the ublue-os tap is available
...
systemctl --user disable dakota-oem-setup.service 2>/dev/null || true
`, vendor, brewInstallCmds)
...
ConditionPathExists=!%h/.config/dakota/oem-setup-done

[Service]
Type=oneshot
ExecStart=/etc/dakota/oem/install-packages.sh
RemainAfterExit=no
...
`
	...
	unitPath := filepath.Join(unitDir, "dakota-oem-setup.service")
	...
	preset := "enable dakota-oem-setup.service\n"
	presetPath := filepath.Join(presetDir, "50-dakota-oem.preset")
