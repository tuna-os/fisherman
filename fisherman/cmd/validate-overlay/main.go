...
func main() {
	...
	fmt.Println("  ✓ selectStorageDriver(scratch, true) -> ('overlay', 'reason')")
	fmt.Println("  ✓ podmanArgs includes: --storage-driver overlay")
	fmt.Println()
	fmt.Println("On BTRFS scratch:")
	fmt.Println("  ✓ selectStorageDriver(scratch, true) -> ('vfs', 'scratch filesystem btrfs does not support overlay')")
	fmt.Println("  ✓ podmanArgs includes: --storage-driver vfs")
	fmt.Println()
	fmt.Println("On tmpfs scratch (like /tmp):")
	fmt.Println("  ✓ selectStorageDriver(scratch, true) -> ('vfs', 'scratch filesystem tmpfs does not support overlay')")
	fmt.Println("  ✓ podmanArgs includes: --storage-driver vfs")
	fmt.Println()
	fmt.Println("If podman probe fails:")
	fmt.Println("  ✓ selectStorageDriver(scratch, true) -> ('vfs', 'podman overlay probe failed: ...')")
	fmt.Println("  ✓ podmanArgs includes: --storage-driver vfs")
	fmt.Println()
	fmt.Println("Non-composefs (e.g., grub2+ostree):")
	fmt.Println("  ✓ selectStorageDriver(scratch, false) -> ('vfs', 'standard containers-storage (non-composefs path)')")
	fmt.Println("  ✓ podmanArgs NOT modified (unchanged backward compatibility)")
	fmt.Println()
}
